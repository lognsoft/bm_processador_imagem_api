...
export default function sessionRoutes(upload: any) {
  ...
  // ✅ NOVO: cria sessão em POST /sessions (alinha com o HTML)
  router.post('/', upload.single('file'), async (req, res) => {
    ...
    try {
      if (!req.file) {
        return res.status(400).json({ ok: false, error: 'Envie "file".', log: log.lines.join('\n') });
      }
      ...
      if (!mimeOk && !['jpeg', 'png', 'webp', 'tiff', 'heic', 'avif'].includes(probe.format || '')) {
        return res.status(415).json({ ok: false, error: 'Formato não suportado.', log: log.lines.join('\n') });
      }

      const sid = crypto.randomBytes(6).toString('hex');
      createSession(sid, { original: req.file.buffer, steps: [], meta: probe });
  ...
  // ✅ Alias legado: POST /sessions/docs (mantido)
  ...
  router.post('/:sid/export', async (req, res) => {
    ...
    try {
      ...
      const out = await log.step(`Encode (${outFormat})`, async () => img.toFormat(outFormat, fmtOpts).toBuffer());
  ...
  router.post('/:sid/savePreset', async (req, res) => {
    ...
    if (publish) {
      ...
      if (!/^[a-z0-9-]{3,64}$/.test(slug)) {
        return res
          .status(400)
          .json({ ok: false, error: 'Slug inválido. Use letras, números e hífens (3–64 chars).' });
      }
      const exists = publicSlugExists(slug);
      if (exists && !allowOverwrite) {
        return res
          .status(409)
          .json({ ok: false, error: `Slug já existe: ${slug}. Envie allowOverwrite:true para substituir.` });
      }
      publishPreset(presetId, slug, name);
      published = { slug };
    }

    return res.json({ ok: true, presetId, name: presetObj.name, stepsCount: s.steps.length, published });
  });
