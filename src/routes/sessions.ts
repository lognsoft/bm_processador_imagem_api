...
import { createSession, getSession, pushStep, replaceStep, removeStep, setPreview, nextReqId } from '../store/sessionsStore.js';
...
export default function sessionRoutes(upload: any) {
  ...
  router.post('/docs', upload.single('file'), async (req, res) => {
    ...
    try {
      ...
      const previewBuf = await sharp(req.file.buffer).toFormat(
        format, format==='png'?{compressionLevel:9}:{quality:90}
      ).toBuffer();
