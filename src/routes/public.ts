...
export default function publicRoutes(upload: any) {
  ...
  router.get('/presets', (_req, res) => {
    const list = listPublicPresets().map(p => ({ slug: p.slug, name: p.name, presetId: p.presetId, updatedAt: p.updatedAt }));
  ...
  router.get('/presets/:slug', (req, res) => {
    ...
    res.json({ ok:true, slug: meta.slug, name: meta.name, presetId: meta.presetId, steps: preset.steps });
  });
  ...
  router.post('/process', upload.single('file'), async (req, res) => {
    ...
    try {
      ...
      if (wantsJson) {
        ...
        return res.json({
          ok: true,
          preset: { slug: meta.slug, name: preset.name, presetId: meta.presetId },
  ...
  router.delete('/presets/:slug', (req, res) => {
    ...
    if (!ok) return res.status(404).json({ ok:false, error: 'Slug não encontrado.' });
